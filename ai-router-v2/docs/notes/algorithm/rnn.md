...
其中，用 $x_t$ 表示第 $t$ 天的价格，作为一个股票预测员，想要预测股票所能依靠的信
...
举一个例子，如果我们想生成: `deep learning is fun`, 那么我们就需要不断地计算这样
的概率：
...
那么我们需要每一次保存前一个时间步的隐藏变量 $H_{t-1}$ , 并且引入一个新的权重
...
不难发现，RNN在时间步过长的时候，仍然会出现梯度消失 or 梯度爆炸的现象，具体如
下：
